...
for cand in "$CHAINS" "$DIR/taskmarket-chains.tsv" "$SKILL_DIR/templates/chains.tsv"; do
...
[ -f taskmarket-dependency-tree.sha256 ] || { echo "missing staged dependency digest" >&2; exit 5; }
STAGED_DEP_TREE="$(tr -d '[:space:]' < taskmarket-dependency-tree.sha256)"
[ "$STAGED_DEP_TREE" = "$PINNED_DEPENDENCY_TREE_SHA256" ] || {
  echo "staged dependency attestation differs from the pack's reviewed digest" >&2
  exit 5
}
if [ -n "${TASKMARKET_DEPENDENCY_TREE_SHA256:-}" ] \
  && [ "$TASKMARKET_DEPENDENCY_TREE_SHA256" != "$PINNED_DEPENDENCY_TREE_SHA256" ]; then
  echo "workflow dependency digest differs from the pack's reviewed digest" >&2
  exit 5
fi
EXPECTED_DEP_TREE="$PINNED_DEPENDENCY_TREE_SHA256"
...
scan_generated_hook() {
  ...
  generated_sources="$(find src -type f -name '*.sol' -print)"
  ...
  while IFS= read -r import_path; do
    case "$import_path" in
      @taskmarket/contracts/src/hooks/base/BaseTMPHook.sol|@taskmarket/contracts/src/interfaces/*|@openzeppelin/contracts/*) ;;
...
preflight_target() {
  local actual_chain code codehash revision usdc defaults default_count remaining hook hook_code hook_support
  local selector_sig selector route route_code route_lower routes=""
  ...
  revision="$(cast call "$DIAMOND" 'diamondVersion()(uint256)' --block "$FORK_BLOCK" --rpc-url "$RPC_URL")" || {
    echo "could not read TaskMarket Diamond revision" >&2; return 1;
  }
  [ "$revision" = "$EXPECTED_REVISION" ] || {
    echo "TaskMarket Diamond revision mismatch: expected=$EXPECTED_REVISION observed=$revision" >&2; return 1;
  }
  usdc="$(cast call "$DIAMOND" 'usdcToken()(address)' --block "$FORK_BLOCK" --rpc-url "$RPC_URL")" || {
    echo "could not read TaskMarket USDC token" >&2; return 1;
  }
  [ "$(printf '%s' "$usdc" | tr '[:upper:]' '[:lower:]')" = "$(printf '%s' "$EXPECTED_USDC" | tr '[:upper:]' '[:lower:]')" ] || {
    echo "TaskMarket USDC mismatch: expected=$EXPECTED_USDC observed=$usdc" >&2; return 1;
  }
  for selector_sig in 'diamondVersion()' 'usdcToken()' 'getDefaultHooks()' 'getTaskHooks(bytes32)'; do
    selector="$(cast sig "$selector_sig")"
    route="$(cast call "$DIAMOND" 'facetAddress(bytes4)(address)' "$selector" --block "$FORK_BLOCK" --rpc-url "$RPC_URL")" || {
      echo "could not resolve Diamond route for $selector_sig" >&2; return 1;
    }
    route_lower="$(printf '%s' "$route" | tr '[:upper:]' '[:lower:]')"
    [ "$route_lower" != "0x0000000000000000000000000000000000000000" ] || {
      echo "Diamond route is unset for $selector_sig" >&2; return 1;
    }
    route_code="$(cast code "$route" --block "$FORK_BLOCK" --rpc-url "$RPC_URL")"
    [ -n "$route_code" ] && [ "$route_code" != "0x" ] || {
      echo "Diamond route has no facet code for $selector_sig: $route" >&2; return 1;
    }
    routes="${routes}${routes:+;}$selector_sig=$route"
  done
  LIVE_DIAMOND_REVISION="$revision"
  LIVE_USDC="$usdc"
  LIVE_SELECTOR_ROUTES="$routes"
  echo "target: $CHAIN chainId=$CHAIN_ID diamond=$DIAMOND codehash=$codehash revision=$revision usdc=$usdc forkBlock=$FORK_BLOCK forkHash=$FORK_BLOCK_HASH rpc=$RPC_LABEL"
  echo "code-backed selector routes: $routes"
...
persist_simulation_evidence() {
  ...
  jq -n \
    ...
    --argjson diamondRevision "$LIVE_DIAMOND_REVISION" --arg usdc "$LIVE_USDC" --arg selectorRoutes "$LIVE_SELECTOR_ROUTES" \
...
if [ "$TESTNET" != "true" ]; then
  ...
  REQUESTED_MAINNET="$(printf '%s\n' "${SKILL_VAR#arm:}" | awk '{for (i=1; i<=NF; i++) if ($i=="chain:base") {print $i; exit}}')"
...
echo "  Diamond revision   $LIVE_DIAMOND_REVISION"
echo "  TaskMarket USDC    $LIVE_USDC"
echo "  hook               $HOOK_ADDR"
...
if command -v jq >/dev/null 2>&1; then
  jq -n \
    ...
    --arg diamondCodehash "$EXPECTED_DIAMOND_CODEHASH" --argjson diamondRevision "$LIVE_DIAMOND_REVISION" \
    --arg usdc "$LIVE_USDC" --arg selectorRoutes "$LIVE_SELECTOR_ROUTES" --arg hook "$HOOK_ADDR" \
