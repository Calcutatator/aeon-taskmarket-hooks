...
description: Brainstorm exactly five buildable TaskMarket V1 hooks with xAI, deduplicated against prior ideas, attempts, deployments, and official or observed Hooklist records.
metadata:
...
5. Require an implementation small enough for one hook contract plus tests, with no oracle/backend unless the trust and stale-data failure mode are explicit. Mark asset-moving, escrow, Diamond-calling, proxy, or low-level-call concepts as `dedicated-build-required`; they can be valid ideas but are not direct inputs to the generic deploy skill.
...
Classify every survivor as exactly one of:

- `v1-direct-hook` — fits the bounded generic deploy skill.
- `v1-separately-funded-sidecar` — valid V1 hook, but moves an explicitly separate asset amount and requires a dedicated reviewed build.
- `v1-companion-router` — buildable only with an additional router/backend/permit flow; name that companion explicitly.
- `core-change-or-v2` — not buildable against V1; reject it from the final five rather than presenting it as ready.

For every class, record `coreEscrowMutation` (must be `false`), the funding path, any X/Y value separation, companion components, and the exact V1 limitation.
...
- Feasibility class, `coreEscrowMutation=false`, funding path, X/Y separation where relevant, and any required companion.
...
- A compact build brief suitable as direct input to `deploy-taskmarket-hook`, or an explicit `dedicated-build-required` label with the reason it exceeds that skill's bounded generator.
