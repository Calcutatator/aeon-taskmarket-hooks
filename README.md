...
|---|---|---|
| `taskmarket-hook-idea` | Uses the xAI Responses API to produce exactly five buildable briefs, deduplicated against local attempts, observed public hooks, and the official registry/open listings when accessible. | Disabled; manual run |
...
Without `arm:`, it never broadcasts. Base Sepolia is the default. Base mainnet additionally requires explicit `chain:base` and `HOOK_MAINNET_OK=1`. Optional secrets are `HOOK_DEPLOYER_PRIVATE_KEY` for a dedicated gas-only deployer and `ETHERSCAN_API_KEY` for best-effort source verification.

The deploy skill provisions Foundry v1.7.1 from checksum-pinned official binaries and uses the exact dependency revisions from `create-taskmarket-hook@0.1.0`. Its runner independently checks canonical Diamond identity, immutable fixtures, the official `BaseTMPHook`, and the complete dependency-tree digest before any key-bearing path. Simulation has no broadcast context; the separate immutable broadcast script checks raw authorization and reads the burner key internally, never as a CLI argument.

## Deliberate limits

The generic deployer is for direct policy and observation hooks. It refuses token transfers, external escrow, callbacks into the TaskMarket Diamond, arbitrary low-level calls, proxies, and child deployments. It does not attach a hook to a task, fund a task, change protocol defaults, or submit registry metadata. It does preserve a schema-shaped draft manifest and names the exact official Hooklist validation commands for a later maintainer-reviewed submission.
